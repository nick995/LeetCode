...
-- and one user who posted two tweets in 2022. The query groups the users by the number 
-- of tweets they posted and displays the number of users in each group.

WITH CTE AS(
    SELECT user_id, COUNT(tweet_id) AS tweet_count
    FROM tweets
    WHERE date_part('year', tweets.tweet_date) = '2022'
    GROUP BY user_id
)
